...
window.onload = function() {
  ...
  function onresize() {
   ...
   // MOVE DOWN!!!!!
   // gl.viewport(0, 0, gl.canvas.width, gl.canvas.height);
    gl.viewport(50, -50, gl.canvas.width, gl.canvas.height);
    ...
    //MAKE BIGGER!!!!!!!
    gl.perspective(28, gl.canvas.width / gl.canvas.height, 0.01, 100);
  ...
  center = oldCenter = new GL.Vector(0.0, 0.0, 0.0);
  ...
  function startDrag(x, y) {
    ...
    console.log("function startDrag(x, y): " + x + "  " + y);
...
//replaced
  ...
  function duringDrag(x, y) {
    ...
    console.log("function duringDrag(x, y) {: " + x + " " + y);
    console.log("duringDrag mode: " + mode);
  ...
  /*      console.log("function duringDrag(x, y) { BEFORE  center.x: " + center.x + "  center.y: " + center.y + "  center.z: " + center.z);

        // MANUALLY ADD ABSOLUTE POSITION COORDINATES 
        if(haveBluetoothConnection){
            var newCenterX = ((xCoordinate / 100) * 1.5) - 0.75;
            var newCenterY = ((yCoordinate / 100) * 1.5) - 0.75;
            var newCenterZ = ((zCoordinate / 100) * 1.5) - 0.75;

            console.log("function duringDrag(x, y) { AFTER  center.x: " + newCenterX + "  center.y: " + newCenterY + "  center.z: " + newCenterZ);
    ...
    console.log("duringDrag(x, y) case MODE_MOVE_SPHERE  center.x: " + center.x + "  center.y: " + center.y + "  center.z: " + center.z);
  ...
  //  console.log("function update(seconds) mode: " + mode);
...
/**************************************************************************************************
********************* Curt add for Web Bluetooth, NN, absolute position ***************************
**************************************************************************************************/

  var havePosition = false;
  var newPageX, newPageY, newPageZ;

  initAbsolutePosition();
  
  function applyAbsolutePosition(){
    //start after Web Bluetooth connect and first coordinate aquisition
    havePosition = false;

    if(updateCoordinatesFlag && haveBluetoothConnection){

        console.log("BEFORE  center.x: " + center.x + "  center.y: " + center.y + "  center.z: " + center.z);
        ...
        console.log("AFTER  center.x: " + newCenterX + "  center.y: " + newCenterY + "  center.z: " + newCenterZ);
